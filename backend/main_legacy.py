...
Estas rutas se mantienen durante la migración a v2.0
"""
...
from database import (
    User, Company, CFDI, Declaration, Subscription, AIConversation,
...
@router.post("/api/companies", response_model=CompanyOut)
def create_company(
...
@router.get("/api/cfdi", response_model=List[CFDIOut])
def list_cfdis(
    company_id: int,
...
@router.get("/api/tax/calculate/{company_id}")
def calculate_taxes(
    company_id: int,
    year: int,
    month: int,
...
@router.get("/api/subscription", response_model=SubscriptionOut)
...
def create_checkout_legacy(
    ...
    price_ids = {
        "godinez": os.getenv('STRIPE_PRICE_GODINEZ', 'price_godinez_monthly'),
        "empresario": os.getenv('STRIPE_PRICE_EMPRESARIO', 'price_empresario_monthly'),
        "sovereign": os.getenv('STRIPE_PRICE_SOVEREIGN', 'price_sovereign_monthly')
    }
...
@router.get("/api/ai/recommendations/{company_id}")
